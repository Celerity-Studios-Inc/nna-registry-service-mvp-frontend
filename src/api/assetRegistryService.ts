...
class AssetRegistryService {
  ...
  private saveToStorage(): void {
    try {
      localStorage.setItem(this.storageKey, JSON.stringify(this.registeredAssets));
    } catch (error) {
  ...
  updateAsset(
    ...
    const index = this.registeredAssets.findIndex(item => item.asset.id === assetId);
   ...
   */
  findAssetsByFingerprint(fingerprint: Partial<AssetFingerprint>): RegisteredAsset[] {
    return this.registeredAssets.filter(item => {
      ...
      for (const key in fingerprint) {
        if (key in fingerprint && key in item.fingerprint) {
          ...
          if (fingerprint[k] !== undefined && fingerprint[k] !== item.fingerprint[k]) {
            return false;
          }
        }
      }
  ...
  findExactAssetMatch(fingerprint: AssetFingerprint): RegisteredAsset | null {
    const matches = this.registeredAssets.filter(item => 
    ...
    return matches.length > 0
      ? matches.sort((a, b) => b.registeredAt.getTime() - a.registeredAt.getTime())[0]
      : null;
  }
  ...
  getAssetsByNNAAddress(nnaAddress: string): RegisteredAsset[] {
    return this.registeredAssets.filter(
      item => item.asset.nnaAddress === nnaAddress ||
  ...
  removeAsset(id: string): boolean {
    ...
    this.registeredAssets = this.registeredAssets.filter(item => item.asset.id !== id);
