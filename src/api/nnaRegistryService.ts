...
class NNARegistryService {
  ...
  private subcategoryRegistry: Map<string, Map<string, CodeMapping>> =
    new Map();
  ...
  private usedSubcategoryCodes: Map<string, Map<string, Set<number>>> =
    new Map();
      ...
      for (const category of categories) {
        ...
        if (category.code && category.name) {
          ...
          if (/^\d+$/.test(alphabeticCode) || alphabeticCode.length !== 3) {
            ...
            alphabeticCode = this.generateAlphabeticCode(
              category.name,
              layerCode
            );
          }
          ...
          if (!this.usedSubcategoryCodes.get(layerCode)!.has(alphabeticCode)) {
            this.usedSubcategoryCodes
              .get(layerCode)!
              .set(alphabeticCode, new Set<number>());
          }
          ...
          for (const subcategory of subcategories) {
            if (subcategory.code && subcategory.name) {
              ...
              if (
                /^\d+$/.test(subAlphabeticCode) ||
                subAlphabeticCode.length !== 3
              ) {
      ...
      if (subRegistry) {
        ...
        while (
          valueArray.some(mapping => {
            ...
            return (
              mapping.alphabeticCode === currentAlphaCode &&
              categoryCode ===
                this.getCategoryForSubcategory(
                  layerCode,
                  mapping.alphabeticCode
                )
            );
          }) &&
          attempt < 100 // Safety limit
   ...
   */
  private getCategoryForSubcategory(
    layerCode: string,
  ...
  private registerCategoryCode(
    ...
    if (usedCodes.size >= 1000) {
      console.error(
        `Layer ${layerCode} has reached the maximum of 1000 category codes`
      );
      return;
    }
    ...
    if (registry.has(alphabeticCode)) {
      console.warn(
        `Alphabetic code ${alphabeticCode} already registered for layer ${layerCode}`
      );
      return;
    }
    ...
    if (usedCodes.has(numericCode)) {
      ...
      console.warn(
        `Numeric code ${numericCode} already in use for layer ${layerCode}, using ${newNumericCode} instead`
      );
  ...
  private registerSubcategoryCode(
    ...
    if (usedCodes.size >= 1000) {
      console.error(
        `Category ${categoryCode} in layer ${layerCode} has reached the maximum of 1000 subcategory codes`
      );
      return;
    }
    ...
    if (registry.has(key)) {
      console.warn(
        `Subcategory code ${key} already registered for layer ${layerCode}`
      );
      return;
    }
    ...
    if (usedCodes.has(numericCode)) {
      ...
      if (newNumericCode >= 1000) {
        ...
        if (newNumericCode >= numericCode) {
          console.error(
            `No available numeric codes for category ${categoryCode} in layer ${layerCode}`
          );
          return;
        }
      }
      ...
      console.warn(
        `Numeric code ${numericCode} already in use for category ${categoryCode} in layer ${layerCode}, using ${newNumericCode} instead`
      );
      numericCode = newNumericCode;
    }
  ...
  private registerAddress(
    ...
    const humanFriendlyName = `${layerCode}.${categoryMapping.alphabeticCode}.${
      subcategoryMapping.alphabeticCode
    }.${sequentialNumber.toString().padStart(3, '0')}`;
    const machineFriendlyAddress = `${layerCode}.${categoryMapping.numericCode
      .toString()
      .padStart(3, '0')}.${subcategoryMapping.numericCode
      .toString()
  ...
  generateHumanFriendlyName(
    ...
    let subcategoryCode = this.getSubcategoryAlphabeticCodeByName(
      layerCode,
      categoryCode,
      subcategoryName
    );
    if (!subcategoryCode) {
      subcategoryCode = this.generateAlphabeticCode(
        subcategoryName,
        layerCode,
        categoryCode
      );
      ...
      const subcategoryNumericCode =
        this.getNextAvailableSubcategoryNumericCode(layerCode, categoryCode);
      this.registerSubcategoryCode(
        layerCode,
        categoryCode,
        subcategoryCode,
    ...
    return `${layerCode}.${categoryCode}.${subcategoryCode}.${sequentialStr}`;
  }
  ...
  generateMachineFriendlyAddress(
    ...
    if (!categoryCode) {
      ...
      categoryNumericCode = this.getNumericCodeByAlphabetic(
        layerCode,
        categoryCode
      );
    }
    ...
    if (!subcategoryCode) {
      ...
      subcategoryNumericCode = this.getNextAvailableSubcategoryNumericCode(
        layerCode,
        categoryCode
      );
      this.registerSubcategoryCode(
      ...
      subcategoryNumericCode = this.getSubcategoryNumericCodeByAlphabetic(
        layerCode,
        categoryCode,
        subcategoryCode
      );
   ...
   */
  private getNextAvailableSubcategoryNumericCode(
    layerCode: string,
    categoryCode: string
  ): number {
    const usedCodes = this.usedSubcategoryCodes
      .get(layerCode)
      ?.get(categoryCode);
    if (!usedCodes) return 1;
   ...
   */
  private getAlphabeticCodeByName(
    layerCode: string,
    name: string
  ): string | null {
    const registry = this.categoryRegistry.get(layerCode);
   ...
   */
  private getNumericCodeByAlphabetic(
    layerCode: string,
    alphabeticCode: string
  ): number {
    const registry = this.categoryRegistry.get(layerCode);
  ...
  convertToMachineFriendly(humanFriendlyName: string): string {
    ...
    const categoryNumericCode = this.getNumericCodeByAlphabetic(
      layerCode,
      categoryCode
    );
  ...
  convertToHumanFriendly(machineFriendlyAddress: string): string {
    ...
    if (parts.length !== 4) {
      console.error(
        'Invalid machine-friendly address format:',
        machineFriendlyAddress
      );
      return '';
    }
    ...
    if (!categoryCode) {
      console.error(
        'Failed to find category code for:',
        machineFriendlyAddress
      );
      return '';
    }
    ...
    if (subcategoryRegistry) {
      ...
      for (const [key, mapping] of entries) {
        ...
        if (
          catCode === categoryCode &&
          mapping.numericCode === subcategoryNumericCode
        ) {
    ...
    if (!subcategoryCode) {
      console.error(
        'Failed to find subcategory code for:',
        machineFriendlyAddress
      );
      return '';
    }

    // Format as human-friendly name
    return `${layerCode}.${categoryCode}.${subcategoryCode}.${sequentialStr}`;
  }
  ...
  getHumanFriendlyName(
    ...
    if (registry) {
      for (const entry of registry) {
        if (
          entry.categoryCode.name.toLowerCase() ===
            categoryName.toLowerCase() &&
          entry.subcategoryCode.name.toLowerCase() ===
            subcategoryName.toLowerCase() &&
          entry.sequentialNumber === sequentialNumber
        ) {
