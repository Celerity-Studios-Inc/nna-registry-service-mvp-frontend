 ...
 */
export const CATEGORY_NUMERIC_MAPPINGS: Record<
  string,
 ...
 * Now derived from flattened taxonomy
 */
export const SUBCATEGORY_NUMERIC_MAPPINGS: Record<
  string,
  Record<string, Record<string, number>>
> = {
  W: Object.fromEntries(
    Object.entries(flattenedTaxonomy.W.subcategories).map(
  ...
  S: Object.fromEntries(
    Object.entries(flattenedTaxonomy.S.subcategories).map(
      ([categoryCode, subcategories]) => [
        categoryCode,
        Object.fromEntries(
          Object.entries(subcategories).map(([subcategoryCode, data]) => [
 ...
 * Now derived from flattened taxonomy
 */
export const CATEGORY_ALPHABETIC_MAPPINGS: Record<
  string,
  Record<number, string>
> = {
  W: Object.fromEntries(
    // Convert the categoriesByMfa mapping which is already in the right format
    Object.entries(flattenedTaxonomy.W.categoriesByMfa).map(
      ([mfaCode, hfnCode]) => [parseInt(mfaCode, 10), hfnCode]
    )
  ),
  S: Object.fromEntries(
    Object.entries(flattenedTaxonomy.S.categoriesByMfa).map(
      ([mfaCode, hfnCode]) => [parseInt(mfaCode, 10), hfnCode]
    )
  ),
  G: {
 ...
 * Now derived from flattened taxonomy
 */
export const SUBCATEGORY_ALPHABETIC_MAPPINGS: Record<
  string,
  Record<number, Record<number, string>>
> = {
  W: Object.fromEntries(
    // Transform the structure to match the required format
    Object.entries(flattenedTaxonomy.W.subcategoriesByMfa).map(
      ([categoryMfa, subcategoryMap]) => [
        parseInt(categoryMfa, 10),
        Object.fromEntries(
          Object.entries(subcategoryMap).map(
            ([subcategoryMfa, subcategoryHfn]) => [
  ...
  S: Object.fromEntries(
    Object.entries(flattenedTaxonomy.S.subcategoriesByMfa).map(
      ([categoryMfa, subcategoryMap]) => [
        parseInt(categoryMfa, 10),
        Object.fromEntries(
          Object.entries(subcategoryMap).map(
