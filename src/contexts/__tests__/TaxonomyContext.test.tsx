...
describe('TaxonomyContext', () => {
  ...
  it('provides taxonomy context to children', () => {
    ...
    expect(useTaxonomy).toHaveBeenCalledWith({ autoLoad: true, showFeedback: true });
  });
  ...
  it('handles custom options', () => {
    ...
    expect(result.current).toEqual(mockTaxonomyHook);
    expect(useTaxonomy).toHaveBeenCalledWith({ autoLoad: false, showFeedback: false });
  });
  ...
  it('throws an error when used outside of provider', () => {
    ...
    expect(result.error?.message).toContain('useTaxonomyContext must be used within a TaxonomyProvider');
  });
});