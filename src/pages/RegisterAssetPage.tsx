...
const RegisterAssetPage: React.FC = () => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        if (
          ...
          console.log(
            'DETECTED S.POP.LGM COMBINATION - Will convert to proper codes for backend'
          );
        }
        ...
        if (
          data.layer === 'S' &&
          data.categoryCode === 'POP' &&
          data.subcategoryCode === 'HPM'
        ) {
          ...
          if (data.mfa !== '2.001.007.001') {
            console.error(
              `WARNING: MFA is incorrect! Expected 2.001.007.001 but got ${data.mfa}`
            );
          }
        }
      }
      ...
      if (data.layer === 'W') {
        ...
        console.log(
          `W layer detected: Using simplified taxonomy service with codes ${data.categoryCode} and ${data.subcategoryCode}`
        );

        // No conversion needed, but add additional logging for verification
        if (data.categoryCode === 'BCH' && data.subcategoryCode === 'SUN') {
          console.log(
            'IMPORTANT - W.BCH.SUN case detected - will use simplified taxonomy service'
          );
          console.log(`MFA should be 5.004.003.001, actual: ${data.mfa}`);
        }
      } else if (
        data.layer === 'S' &&
        (data.categoryCode === '001' || /^\d+$/.test(data.categoryCode))
      ) {
        // Special case for S layer POP category
        convertedCategory = 'POP';
        console.log(
          `CRITICAL PATH: Force mapped category code ${data.categoryCode} → POP for Stars layer`
        );

        // Convert subcategory code to alphabetic form if needed
        if (data.categoryCode === 'POP' || data.categoryCode === '001') {
          if (
            data.subcategoryCode === '007' ||
            (/^\d+$/.test(data.subcategoryCode) &&
              data.subcategoryCode === '007')
          ) {
            convertedSubcategory = 'HPM';
            console.log(
              `CRITICAL PATH: Force mapped subcategory code ${data.subcategoryCode} → HPM for S.POP layer`
            );
          } else {
      ...
      console.log(
        `Subcategory: ${data.subcategoryCode} → ${convertedSubcategory}`
      );
      ...
      if (
        ...
        console.log(
          `These should be sending: layer=S, category=POP, subcategory=HPM to the backend`
        );

        // Check if the conversion is correct
        if (convertedCategory !== 'POP' || convertedSubcategory !== 'HPM') {
          console.error(
            'WARNING: S.POP.HPM conversion failed! Backend will likely reject this request.'
          );
        } else {
          console.log(
            '✅ S.POP.HPM conversion successful - should work with backend API'
          );
        }
      }
      ...
      const assetData = {
        ...
        metadata: {
          ...
          ...(() => {
            ...
            if (!mfa) {
              console.warn(
                `Taxonomy service failed to convert ${hfn}, using fallback`
              );
              ...
              if (
                ...
                data.layer === 'W' &&
                data.categoryCode === 'BCH' &&
                data.subcategoryCode === 'SUN'
              ) {
                finalMfa = '5.004.003.001';
              } else {
                // Default fallback
                const layerCode = taxonomyService.getLayerNumericCode(
                  data.layer
                );
  ...
  const handleLayerSelect = (layer: LayerOption, isDoubleClick?: boolean) => {
    ...
    if (layerSelectionThrottleRef.current.throttleTimeout) {
    ...
    const layerChangeId = `lc_${now}_${Math.random()
      .toString(36)
      .substr(2, 5)}`;
    console.log(
      `[LAYER SELECT ${layerChangeId}] Starting layer switch operation to ${layer.code}`
    );

    // DIAGNOSTIC: Log the taxonomy context state before layer switch
    console.log(
      `[LAYER SELECT ${layerChangeId}] Before switch - taxonomyContext.selectedLayer: ${taxonomyContext.selectedLayer}`
    );
    console.log(
      `[LAYER SELECT ${layerChangeId}] Before switch - taxonomyContext.categories: ${JSON.stringify(
        taxonomyContext.categories.map(c => c.code)
      )}`
    );
    console.log(
      `[LAYER SELECT ${layerChangeId}] Before switch - taxonomyContext.selectedCategory: ${taxonomyContext.selectedCategory}`
    );
    ...
    try {
      ...
      console.log(
        `[LAYER SELECT ${layerChangeId}] Dispatched layerChanged event for ${layer.code}`
      );
    } catch (e) {
      console.warn(
        `[LAYER SELECT ${layerChangeId}] Failed to dispatch layer change event:`,
        e
      );
    }
    ...
    console.log(
      `[LAYER SELECT ${layerChangeId}] Updating form values for layer ${layer.code}`
    );
    ...
    console.log(
      `[LAYER SELECT ${layerChangeId}] Clearing all category and subcategory form state`
    );
    ...
    try {
      ...
      for (let i = 0; i < sessionStorage.length; i++) {
        ...
        if (
          key &&
          (key.startsWith('originalSubcategory_') ||
      ...
      keysToRemove.forEach(key => {
        ...
        console.log(
          `[LAYER SELECT ${layerChangeId}] Removed session storage key: ${key}`
        );
      });
    } catch (e) {
      console.warn(
        `[LAYER SELECT ${layerChangeId}] Failed to clear subcategories from session storage:`,
        e
      );
    }
    ...
    try {
      ...
      console.log(
        `[LAYER SELECT ${layerChangeId}] Direct service call returned ${directCategories.length} categories for ${layer.code}`
      );

      // Store the layer and category data in session storage for emergency recovery
      if (directCategories.length > 0) {
        sessionStorage.setItem(
          `directCategories_${layer.code}`,
          JSON.stringify(directCategories)
        );
        console.log(
          `[LAYER SELECT ${layerChangeId}] Stored ${directCategories.length} categories in session storage`
        );
      ...
      console.warn(
        `[LAYER SELECT ${layerChangeId}] Failed to prefetch categories:`,
        e
      );
    }
    ...
    setTimeout(() => {
      console.log(
        `[LAYER SELECT ${layerChangeId}] Setting taxonomy context layer to ${layer.code}`
      );
      ...
      setTimeout(() => {
        console.log(
          `[LAYER SELECT ${layerChangeId}] Forcing reload of categories for layer ${layer.code}`
        );
        ...
        setTimeout(() => {
          ...
          if (taxonomyContext.categories.length === 0) {
            console.log(
              `[LAYER SELECT ${layerChangeId}] Categories still not loaded, triggering emergency reload`
            );
            ...
            console.log(
              `[LAYER SELECT ${layerChangeId}] Categories loaded successfully: ${taxonomyContext.categories.length} items`
            );
          }
        }, 300);
      }, 100);
    ...
    setTimeout(() => {
      console.log(
        `[LAYER SELECT ${layerChangeId}] After switch (500ms) - taxonomyContext.selectedLayer: ${taxonomyContext.selectedLayer}`
      );
      console.log(
        `[LAYER SELECT ${layerChangeId}] After switch (500ms) - taxonomyContext.categories: ${JSON.stringify(
          taxonomyContext.categories.map(c => c.code)
        )}`
      );
      console.log(
        `[LAYER SELECT ${layerChangeId}] After switch (500ms) - form values: ${JSON.stringify(
          {
    ...
    if (isDoubleClick) {
      console.log(
        `[LAYER SELECT ${layerChangeId}] Double-click detected but NOT auto-advancing to prevent Step 2 skipping`
      );
      console.log(
        `[LAYER SELECT ${layerChangeId}] This fixes the issue where subcategory selection was being bypassed`
      );
    }
  };
  ...
  const handleCategorySelect = (category: CategoryOption) => {
    ...
    try {
      sessionStorage.removeItem(
        `originalSubcategory_${watchLayer}_${category.code}`
      );
    } catch (e) {
  ...
  const handleSubcategorySelect = (
    ...
    try {
      sessionStorage.setItem(
        `originalSubcategory_${watchLayer}_${watchCategoryCode}`,
        subcategory.code
      );
      console.log(
        `Stored backup subcategory in session storage: ${subcategory.code}`
      );
    } catch (e) {
  ...
  const [originalSubcategoryCode, setOriginalSubcategoryCode] =
    useState<string>('');
  ...
  const _handleNNAAddressChange = (
    ...
    if (originalSubcategory) {
      ...
      console.log(
        `Stored original subcategory code: ${originalSubcategory} for display override`
      );
    }
  };
  ...
  const [layerSwitchLog, setLayerSwitchLog] = useState<
    {
      timestamp: number;
      layer: string;
      categories: string[];
  ...
  useEffect(() => {
    if (watchLayer) {
      ...
      console.log(
        `[LAYER SWITCH LOG] History length: ${layerSwitchLog.length + 1}`
      );
      ...
      const verificationTimer = setTimeout(() => {
        ...
        console.log(
          `Categories: ${
            taxonomyContext.categories.length
          } (${taxonomyContext.categories.map(c => c.code).join(', ')})`
        );
        console.log(
          `Subcategories: ${
            taxonomyContext.subcategories.length
          } (${taxonomyContext.subcategories.map(s => s.code).join(', ')})`
        );
        console.log(`Selected category: ${taxonomyContext.selectedCategory}`);
        console.log(
          `Selected subcategory: ${taxonomyContext.selectedSubcategory}`
        );
      }, 1000);
    ...
    watchLayer,
    taxonomyContext.categories,
    taxonomyContext.subcategories,
    layerSwitchLog.length,
    taxonomyContext.selectedCategory,
    taxonomyContext.selectedSubcategory,
  ]);
  ...
  useEffect(() => {
    if (formData.layer && formData.categoryCode && formData.subcategoryCode) {
      ...
      const newHfn = `${formData.layer}.${formData.categoryCode}.${
      ...
      try {
        ...
        if (taxonomyContext.hfn === newHfn && taxonomyContext.mfa) {
          ...
          if (newMfa) {
            setValue('mfa', newMfa);
            console.log(
              `SimpleTaxonomyService HFN to MFA conversion: ${newHfn} -> ${newMfa}`
            );
          } else {
            // If conversion fails, use a fallback format
            console.error(
              `Conversion failed for ${newHfn}, using fallback format`
            );
            ...
            if (
              formData.layer === 'S' &&
              formData.categoryCode === 'POP' &&
              formData.subcategoryCode === 'HPM'
            ) {
              fallbackMfa = '2.001.007.001'; // Special case for S.POP.HPM
            } else if (
              formData.layer === 'W' &&
              formData.categoryCode === 'BCH' &&
              formData.subcategoryCode === 'SUN'
            ) {
        ...
        return (
          <LayerSelector
            ...
            onLayerSelect={(layer, isDoubleClick) => {
              console.log(
                `RegisterAssetPage received layer select: ${layer}, isDoubleClick=${isDoubleClick}`
              );
        ...
        return (
          <SimpleTaxonomySelection
            ...
            onCategorySelect={categoryCode => {
              const category = taxonomyService
                .getCategories(watchLayer)
            ...
            onSubcategorySelect={(subcategoryCode, isDoubleClick) => {
              console.log(
                `Subcategory selected: ${subcategoryCode}, isDoubleClick=${isDoubleClick}`
              );
              ...
              if (subcategory) {
                ...
                handleSubcategorySelect(
                  ...
                  isDoubleClick
                ); // Pass the isDoubleClick parameter for auto-advancing
              } else {
                console.error(
                  `Could not find subcategory ${subcategoryCode} in layer ${watchLayer}, category ${watchCategoryCode}`
                );
              }
            }}
          />
          ...
          return (
            <Paper sx={{ p: 3, mb: 4 }}>
              ...
              <Typography variant="body2" color="text.secondary" paragraph>
                A composite asset references other existing assets. Select the
                components that make up this composite asset.
              </Typography>
              <Divider sx={{ mb: 3 }} />
                ...
                <Grid container spacing={3}>
                  ...
                  <Grid item xs={12}>
                    ...
                    >
                      ...
                      <Typography variant="body2" sx={{ mb: 2 }}>
                        Select the origin of this asset. This is a required
                        field for asset registration.
                      </Typography>
                  ...
                  <Grid item xs={12}>
                    ...
                    >
                      ...
                      <FormControl fullWidth sx={{ mb: 1 }} required>
                        <InputLabel id="source-label-composite">
                          Source
                        </InputLabel>
                        <Select
        ...
        return (
          <ReviewSubmit
            assetData={{
              ...
              uploadedFiles: uploadedFiles,
              tags: getValues('tags'),
              components: isCompositeLayer
                ? getValues('layerSpecificData.components')
                : undefined,
            }}
  ...
  const renderSuccessScreen = () => {
    if (!createdAsset) {
      console.error(
        'Attempted to render success screen but createdAsset is null'
      );
      return (
        <Box sx={{ textAlign: 'center', py: 4 }}>
          ...
          <Typography>
            Your asset has been created but details are not available. Please
    ...
    if (originalSubcategoryCode) {
      ...
      console.log(
        `Using originalSubcategoryCode prop: ${originalSubcategoryCode}`
      );
    }
    // Then try retrieving from sessionStorage
      ...
      try {
        const storedSubcategory = sessionStorage.getItem(
          `originalSubcategory_${layer}_${category}`
        );
        if (storedSubcategory) {
          subcategory = storedSubcategory;
          console.log(
            `Retrieved original subcategory from sessionStorage: ${subcategory}`
          );
        }
      } catch (e) {
    ...
    console.log(
      `Using layer=${layer}, category=${category}, subcategory=${subcategory}, sequential=${sequential}`
    );
    ...
    try {
      ...
      try {
        ...
        if (serviceMfa) {
          ...
          console.log(
            `Fallback: Generated MFA using service directly: ${formattedMfa}`
          );
        }
      } catch (secondError) {
        console.error(
          'Both formatter and service failed to convert HFN to MFA:',
          secondError
        );
      }
    }
    ...
    if (!displayMfa) {
      console.warn(
        'All conversion methods failed, using special case fallbacks'
      );
      ...
      const formattedSequential =
        taxonomyFormatter.formatSequential(sequential);
      ...
      if (
        ...
        normalizedLayer === 'W' &&
        normalizedCategory === 'BCH' &&
        normalizedSubcategory === 'SUN'
      ) {
        displayMfa = `5.004.003.${formattedSequential}`;
      } else {
        // Default fallback based on layer
        const layerCode =
          taxonomyFormatter.getLayerCode(normalizedLayer) || '0';
    ...
    console.log(
      'IMPORTANT: Created asset is using simplified taxonomy service:'
    );
    ...
    console.log(
      `Success screen showing MFA: ${displayMfa} and HFN: ${displayHfn} from asset:`,
      createdAsset
    );

    const layerName =
      createdAsset.metadata?.layerName || `Layer ${createdAsset.layer}`;
    ...
    const isImage =
      ...
      ((displayFile.contentType &&
        displayFile.contentType.startsWith('image/')) ||
    ...
    const isAudio =
      displayFile &&
      ((displayFile.contentType &&
        displayFile.contentType.startsWith('audio/')) ||
        (hasMimeType(displayFile) &&
    ...
    const isVideo =
      displayFile &&
      ((displayFile.contentType &&
        displayFile.contentType.startsWith('video/')) ||
        (hasMimeType(displayFile) &&
    ...
    const isPdf =
      ...
      ((displayFile.contentType &&
        ...
        (hasMimeType(displayFile) &&
          displayFile.mimeType === 'application/pdf'));
                ...
                {displayFile ? (
                  ...
                  >
                    ...
                    {isPdf && displayFile?.url && (
                      ...
                      >
                        <svg width="80" height="80" viewBox="0 0 24 24">
                          <path
                            fill="#f44336"
                            d="M19,3H5C3.9,3,3,3.9,3,5v14c0,1.1,0.9,2,2,2h14c1.1,0,2-0.9,2-2V5C21,3.9,20.1,3,19,3z M9.5,15H8V13h1.5V15z M12.5,15h-1.5V13h1.5V15z M15.5,15H14V13h1.5V15z M16,10H8V8h8V10z"
                          />
                        </svg>
                    ...
                    {displayFile && (
                      ...
                      >
                        {displayFile?.filename} (
                        {displayFile?.size
                          ? `${Math.round(displayFile.size / 1024)} KB`
                          : 'Unknown size'}
                        )
                      </Typography>
                    )}
                  ...
                  >
                    ...
                    <Typography variant="caption" color="error" sx={{ mt: 1 }}>
                      Troubleshooting: Asset files may still be processing or
                      image URL may be incorrect.
                    </Typography>
                    {createdAsset.files &&
                      createdAsset.files.length > 0 &&
                      createdAsset.files[0].url && (
                        <Button
                          variant="outlined"
                          size="small"
                    ...
                    <Grid item xs={12}>
                      ...
                      <Typography variant="body1" fontWeight="bold">
                        {(() => {
                          ...
                          const layer = createdAsset.layer;
                          return `${
                            layerNames[layer] || layerName || `Layer ${layer}`
                          } (${layer})`;
                        })()}
                      </Typography>
                    </Grid>
        ...
        {showSuccessPage && createdAsset ? (
          ...
          <>
            ...
            <Typography
              ...
              color="text.secondary"
            >
              Register a new asset in the NNA Registry. Complete each step to
              create your asset.
            </Typography>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setValue(
                          'files',
                          currentFiles.filter(
                            f => f.name !== potentialDuplicate.file.name
                          )
                        );
                        setPotentialDuplicate(null);
                      }}
              ...
              >
                ...
                <Box mt={1}>
                  ...
                  <Typography variant="caption" component="div">
                    NNA Address:{' '}
                    {potentialDuplicate.asset.nnaAddress ||
                      potentialDuplicate.asset.metadata?.machineFriendlyAddress}
                  </Typography>
                </Box>
              </Alert>
            )}
            ...
            {success && !showSuccessPage && (
              <Alert severity="success" sx={{ mb: 3 }}>
                Asset created successfully! You can create another asset or view
                your assets.
              </Alert>
            )}
            ...
            <FormProvider {...methods}>
              ...
              {(activeStep <
                getSteps(isTrainingLayer, isCompositeLayer).length - 1 ||
                (isTrainingLayer &&
                  activeStep <
                    getSteps(isTrainingLayer, isCompositeLayer).length - 2) ||
                (isCompositeLayer &&
                  activeStep <
                    getSteps(isTrainingLayer, isCompositeLayer).length -
                      2)) && (
                <Box
                  sx={{
                    display: 'flex',
