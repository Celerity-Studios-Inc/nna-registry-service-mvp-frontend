...
const TaxonomyDebug: React.FC = () => {
  ...
  const [specialMappings, setSpecialMappings] = useState<{ hfn: string; mfa: string; success: boolean }[]>([]);
  ...
  return (
    <Paper elevation={3} sx={{ p: 3, m: 2 }}>
      <Typography variant="h5" gutterBottom>Taxonomy Debug Information</Typography>
      
      <Box sx={{ mb: 4 }}>
        <Typography variant="h6" gutterBottom>Special Mappings Test</Typography>
        <Grid container spacing={2}>
          {specialMappings.map((mapping, index) => (
            <Grid item xs={12} key={index}>
              <Paper variant="outlined" sx={{ p: 2 }}>
                <Typography variant="subtitle1">{mapping.hfn} → {mapping.mfa}</Typography>
                <Chip 
      ...
      <Typography variant="h6" gutterBottom>Layer Information</Typography>
          ...
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            <Typography>
              ...
              {debugInfo[layer]?.error 
                ? <span style={{ color: 'red' }}> Error: {debugInfo[layer].error}</span>
                : ` ${debugInfo[layer]?.categoryCount || 0} categories, ${debugInfo[layer]?.lookupKeys || 0} lookup keys`
              }
            </Typography>
          </AccordionSummary>
          <AccordionDetails>
            <Box>
              ...
              <Typography variant="subtitle2" gutterBottom>Subcategories by Category:</Typography>
              {debugInfo[layer]?.subcategories && Object.keys(debugInfo[layer].subcategories).map(cat => (
                <Box key={cat} sx={{ mb: 2 }}>
                  <Typography variant="body2" sx={{ fontWeight: 'bold' }}>{cat} ({debugInfo[layer].subcategories[cat].count}):</Typography>
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, ml: 2 }}>
                    {debugInfo[layer].subcategories[cat].list.map((sub: string) => (
      ...
      <Typography variant="h6" gutterBottom>Raw Layer Lookups Data</Typography>
      <Accordion>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
