...
import { initializeTaxonomy, isTaxonomyInitialized } from '../../../services/taxonomyInitializer';
...
describe('TaxonomyInitProvider', () => {
  ...
  it('should render children when taxonomy is initialized', async () => {
    ...
    expect(screen.queryByText(/Loading taxonomy data/i)).not.toBeInTheDocument();
  ...
  it('should show error when initialization fails', async () => {
    ...
    jest.requireMock('../../../services/taxonomyInitializer').getTaxonomyInitError = getTaxonomyInitErrorMock;
    ...
    await waitFor(() => {
      expect(screen.getByText(/Error Loading Taxonomy Data/i)).toBeInTheDocument();
