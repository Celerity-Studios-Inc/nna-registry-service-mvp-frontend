...
const isPreviewable = (
  file: File | string | { url: string; type?: string; [key: string]: any }
) => {
  ...
  try {
    ...
    if (typeof file === 'string') {
      ...
      return (
        ...
        url.includes('gcpStorageUrl') ||
        url.startsWith('blob:') || // Handle blob URLs
        url.includes('storage.googleapis.com')
      ); // Handle GCP storage URLs
    }
    ...
    if (typeof file === 'object' && file !== null && 'url' in file) {
      ...
      if (!file.url) {
        console.warn(
          'Object with url property has null or undefined url:',
          file
        );
      ...
      return (
        ...
        urlToCheck.endsWith('.gif') ||
        urlToCheck.endsWith('.webp') ||
        urlToCheck.endsWith('.mp4') ||
        urlToCheck.endsWith('.webm') ||
        urlToCheck.endsWith('.mp3') ||
        urlToCheck.endsWith('.wav') ||
        urlToCheck.endsWith('.ogg') ||
        urlToCheck.endsWith('.pdf') ||
        urlToCheck.startsWith('data:image/') ||
        urlToCheck.startsWith('data:video/') ||
        urlToCheck.startsWith('data:audio/') ||
        urlToCheck.includes('gcpStorageUrl') ||
        ...
        urlToCheck.includes('/uploads/')
      ); // Handle upload directories
    }
...
const FilePreview: React.FC<FilePreviewProps> = ({
  ...
  React.useEffect(() => {
    ...
    try {
        ...
        if (fileName && typeof fileName === 'string') {
          ...
          for (let i = 0; i < images.length; i++) {
            ...
            if (img.alt === fileName && img.src) {
              console.log(
                `Found image with matching alt text, using its src:`,
                img.src
              );
      ...
      console.warn(
        'Could not extract a valid URL from the provided file object:',
        file
      );
  ...
  const renderPreview = () => {
    ...
    try {
      ...
      if (
        ...
        file.url.includes('gcpStorageUrl')
      ) {
