...
const FileUploader: React.FC<FileUploaderProps> = ({
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    setIsUploading(false);
  }, [files, hasFileAccess, onFilesAdded, uploadFiles, uploadImmediately, validateFiles]);
  ...
  const fileSummary = useMemo(() => {
    ...
    const pendingFiles = fileStatuses.filter(s => s.status === 'pending').length;
    const uploadingFiles = fileStatuses.filter(s => s.status === 'uploading').length;
    const successFiles = fileStatuses.filter(s => s.status === 'success').length;
        ...
        sx={{
          ...
          backgroundColor: isDragActive
            ...
            : isUploading
              ...
              : 'background.paper',
          cursor: files.length >= maxFiles || isUploading ? 'default' : 'pointer',
          transition: 'all 0.2s ease',
          textAlign: 'center',
          position: 'relative',
          '&:hover': files.length >= maxFiles || isUploading
            ? {}
            : {
                  ...
                  <ListItemText 
                    primary={
                      <Typography 
                        ...
                        component="span" 
                        sx={{ 
                          fontWeight: status.status === 'success' ? 'medium' : 'regular',
                      ...
                      <span>
                        ...
                        {status.status === 'success' && (
                          <Typography component="span" variant="caption" color="success.main" sx={{ display: 'flex', alignItems: 'center' }}>
                            <SuccessIcon fontSize="small" sx={{ mr: 0.5, width: 14, height: 14 }} />
                            Complete
                          </Typography>
                        )}
                        {status.status === 'error' && (
                          <Typography component="span" variant="caption" color="error" sx={{ display: 'flex', alignItems: 'center' }}>
                            <ErrorIcon fontSize="small" sx={{ mr: 0.5, width: 14, height: 14 }} />
                            {status.error || 'Failed'}
                          </Typography>
                        )}
