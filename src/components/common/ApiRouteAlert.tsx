  ...
  useEffect(() => {
    ...
    const checkApiRouting = async () => {
      try {
        ...
        if (isHtml) {
          console.error(
            'API routing error detected: /api/health endpoint returned HTML'
          );
        ...
        if (
          ...
          console.error(
            'API routing error detected: /api/health endpoint returned HTML content'
          );
          setIsHtmlResponse(true);
          // Store in localStorage to persist the warning
          localStorage.setItem('apiRoutingError', 'true');
        ...
        try {
          ...
          console.error(
            'API routing warning: /api/health endpoint returned invalid JSON'
          );
          setIsHtmlResponse(true);
          // Store in localStorage to persist the warning
          localStorage.setItem('apiRoutingError', 'true');
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p>
          The server is returning HTML instead of JSON for API endpoints. This
          happens when the local production server is misconfigured.
        </p>
        <p>
