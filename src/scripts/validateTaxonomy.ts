...
const taxonomyPath = path.resolve(
  __dirname,
  '../assets/enriched_nna_layer_taxonomy_v1.3.json'
);
...
for (const layer in taxonomy) {
  ...
  for (const category in categories) {
    ...
    if (!/^[A-Z]{3}$/.test(category)) {
      errors.push(
        `Category ${category} in layer ${layer} is not a 3-letter uppercase alphabetic code`
      );
      stats.issues++;
    }
    ...
    for (const subcategory of subcategories) {
      ...
      if (!subcategory.code) {
        errors.push(
          `Subcategory in ${layer}.${category} is missing code property`
        );
        stats.issues++;
      }
      ...
      if (!subcategory.numericCode) {
        errors.push(
          `Subcategory ${
            subcategory.code || 'unknown'
          } in ${layer}.${category} is missing numericCode property`
        );
        stats.issues++;
      }
      ...
      if (!subcategory.name) {
        errors.push(
          `Subcategory ${
            subcategory.code || 'unknown'
          } in ${layer}.${category} is missing name property`
        );
        stats.issues++;
      }
      ...
      if (subcategory.code && !/^[A-Z]{3}$/.test(subcategory.code)) {
        errors.push(
          `Subcategory code ${subcategory.code} in ${layer}.${category} is not a 3-letter uppercase alphabetic code`
        );
        stats.issues++;
      }
      ...
      if (subcategory.numericCode && !/^\d{3}$/.test(subcategory.numericCode)) {
        errors.push(
          `Subcategory numericCode ${subcategory.numericCode} in ${layer}.${category}.${subcategory.code} is not a 3-digit string`
        );
        stats.issues++;
      }
    }
...
if (errors.length === 0 && warnings.length === 0) {
  ...
  console.log(
    `❌ Taxonomy validation failed with ${errors.length} errors and ${warnings.length} warnings.`
  );
  process.exit(1);
}
