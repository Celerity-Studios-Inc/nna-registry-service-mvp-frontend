 ...
 */
export function flattenTaxonomy(source: TaxonomyData = taxonomyData as unknown as TaxonomyData): FlattenedTaxonomy {
    ...
    for (const categoryKey in layerInfo.categories) {
      ...
      if (/^\d+$/.test(categoryKey)) {
        ...
        if (category.numericCode !== undefined) {
          ...
          const position = Object.keys(layerInfo.categories).indexOf(categoryKey) + 1;
      ...
      for (const subcategoryKey in category.subcategories) {
        ...
        if (/^\d+$/.test(subcategoryKey)) {
          ...
          if (subcategory.numericCode !== undefined) {
            subcategoryMfaCode = String(subcategory.numericCode).padStart(3, '0');
          } else {
            // Derive from position (not ideal, but a fallback)
            const position = Object.keys(category.subcategories).indexOf(subcategoryKey) + 1;
            subcategoryMfaCode = String(position).padStart(3, '0');
          }
        }

        // Store the subcategory
        flattened[layerCode].subcategories[categoryHfnCode][subcategoryHfnCode] = {
        ...
        // Store the reverse mapping
        flattened[layerCode].subcategoriesByMfa[categoryMfaCode][subcategoryMfaCode] = subcategoryHfnCode;
      }
    }
  }
  ...
  return {
    ...
    subcategories: {
      "POP": {
        ...
        "LGF": { mfaCode: "004", hfnCode: "LGF", name: "Pop_Legend_Female_Stars" },
