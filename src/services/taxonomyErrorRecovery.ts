...
type TaxonomyItemArray = Array<{ code: string; numericCode: string; name: string }>;
...
export function getFallbackCategories(layer: string): TaxonomyItem[] {
  ...
  if (errorCount === 1) {
    logger.warn(`Using fallback categories for layer ${layer} (attempt ${errorCount})`);
  }
 ...
 */
export function getFallbackSubcategories(layer: string, category: string): TaxonomyItem[] {
  const key = `subcategories-${layer}.${category}`;
  const errorCount = (errorTrackingMap.get(key) || 0) + 1;
  errorTrackingMap.set(key, errorCount);
  
  if (errorCount === 1) {
    logger.warn(`Using fallback subcategories for ${layer}.${category} (attempt ${errorCount})`);
  }
 ...
 */
export function getFallbackMFA(layer: string, category: string, subcategory: string, sequential: string, fileType?: string): string {
  const key = `mfa-${layer}.${category}.${subcategory}`;
  const errorCount = (errorTrackingMap.get(key) || 0) + 1;
  errorTrackingMap.set(key, errorCount);

  if (errorCount === 1) {
    logger.warn(`Using fallback MFA mapping for ${layer}.${category}.${subcategory} (attempt ${errorCount})`);
  }
 ...
 */
export function hasReachedMaxRetries(operationType: string, identifier: string): boolean {
 ...
 */
export function incrementRetryCount(operationType: string, identifier: string): number {
  const key = `${operationType}-${identifier}`;
 ...
 */
export function resetRetryCount(operationType?: string, identifier?: string): void {
