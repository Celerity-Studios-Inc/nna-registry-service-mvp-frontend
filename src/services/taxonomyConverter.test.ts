...
describe('TaxonomyConverter', () => {
  beforeEach(() => {
    ...
    // Mock category data
    (taxonomyService.getCategories as jest.Mock).mockImplementation((layer: string) => {
      if (layer === 'S') {
        return [
    ...
    (taxonomyService.getSubcategories as jest.Mock).mockImplementation((layer: string, categoryCode: string) => {
  ...
  describe('getSubcategoryName', () => {
    it('should convert alphabetic subcategory code to name', () => {
      expect(TaxonomyConverter.getSubcategoryName('S', 'POP', 'HPM')).toBe('Pop_Hipster_Male_Stars');
    });

    it('should convert numeric subcategory code to name', () => {
      expect(TaxonomyConverter.getSubcategoryName('S', 'POP', '007')).toBe('Pop_Hipster_Male_Stars');
    });
  });
  ...
  describe('getBackendSubcategoryValue', () => {
    it('should return subcategory name for backend API', () => {
      expect(TaxonomyConverter.getBackendSubcategoryValue('S', 'POP', 'HPM')).toBe('Pop_Hipster_Male_Stars');
    });

    it('should work with numeric codes too', () => {
      expect(TaxonomyConverter.getBackendSubcategoryValue('S', '001', '007')).toBe('Pop_Hipster_Male_Stars');
    });
  });

  describe('isValidCombination', () => {
    it('should validate a correct layer/category/subcategory combination', () => {
      expect(TaxonomyConverter.isValidCombination('S', 'POP', 'HPM')).toBe(true);
    });

    it('should reject an invalid layer/category/subcategory combination', () => {
      expect(TaxonomyConverter.isValidCombination('S', 'POP', 'XYZ')).toBe(false);
    });
  });
  ...
  describe('getAlphabeticCode', () => {
    it('should add the getAlphabeticCode method to support the existing codebase', () => {
      ...
      expect(TaxonomyConverter.getAlphabeticCode('S', 'category', '001')).toBe('POP');
      
      // Test it works for subcategory
      expect(TaxonomyConverter.getAlphabeticCode('S', 'subcategory', '007', 'POP')).toBe('HPM');
    });
  });
});