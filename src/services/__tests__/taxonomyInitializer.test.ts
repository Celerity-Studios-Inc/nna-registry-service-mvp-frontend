...
import flattenedTaxonomy, { generateWorldLayerLookupTable, generateStarLayerLookupTable } from '../../utils/taxonomyFlattener';
  ...
  it('should successfully initialize when taxonomy data is valid', async () => {
    ...
    (taxonomyService.getSubcategories as jest.Mock).mockImplementation((layer, category) => {
      if (layer === 'W' && category === 'BCH') {
        return Object.entries(worldTaxonomy.subcategories.BCH).map(([code, data]) => ({
          code: code, 
          name: data.name, 
          numericCode: data.mfaCode
        }));
      } else if (layer === 'S' && category === 'POP') {
        return Object.entries(starTaxonomy.subcategories.POP).map(([code, data]) => ({
          code: code, 
          name: data.name, 
          numericCode: data.mfaCode
        }));
    ...
    (taxonomyService.convertHFNtoMFA as jest.Mock).mockImplementation((hfn) => {
      ...
      if (layer === 'W') {
        ...
        const categoryCode = worldTaxonomy.categories[category]?.mfaCode || '000';
        const subcategoryCode = worldTaxonomy.subcategories[category]?.[subcategory]?.mfaCode || '000';
        return `${layerCode}.${categoryCode}.${subcategoryCode}.${sequential}`;
      } else if (layer === 'S') {
        const layerCode = starTaxonomy.mfaCode;
        const categoryCode = starTaxonomy.categories[category]?.mfaCode || '000';
        const subcategoryCode = starTaxonomy.subcategories[category]?.[subcategory]?.mfaCode || '000';
        return `${layerCode}.${categoryCode}.${subcategoryCode}.${sequential}`;
      }
