...
describe('useTaxonomy hook', () => {
  ...
  beforeEach(() => {
    ...
    (
      taxonomyErrorRecovery.getFallbackSubcategories as jest.Mock
    ).mockReturnValue([
  ...
  it('should generate HFN and MFA when all selections are made', async () => {
    ...
    (taxonomyService.convertHFNtoMFA as jest.Mock).mockReturnValue(
      '5.001.001.001'
    );
  ...
  it('should provide fallback data when categories fail to load', async () => {
    ...
    const { result } = renderHook(() => useTaxonomy({ autoLoad: true }), {
      wrapper: Wrapper,
    });
