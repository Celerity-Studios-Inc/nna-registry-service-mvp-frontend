...
jest.mock('../../taxonomyLookup', () => {
  ...
  const mockLookup = {
    ...
    G_categories: [
      { code: 'CAT', name: 'Category', numericCode: 1 }
    ],
    S_categories: [
    ...
    S_POP_subcategories: [
      { code: 'HPM', name: 'Happy Mood', numericCode: 7 }
    ],
    G_CAT_subcategories: [
      { code: 'SUB', name: 'Subcategory', numericCode: 1 }
    ],
    S_RCK_subcategories: [
      { code: 'BAS', name: 'Base', numericCode: 1 }
    ]
...
describe('SimpleTaxonomyService', () => {
  ...
  beforeEach(() => {
    ...
    (taxonomyServiceEnhanced.getCategories as jest.Mock).mockImplementation((layer) => {
    ...
    (taxonomyServiceEnhanced.getSubcategories as jest.Mock).mockImplementation((layer, category) => {
    ...
    (taxonomyServiceEnhanced.convertHFNtoMFA as jest.Mock).mockImplementation((hfn) => {
      ...
      if (parts.length >= 4) {
        ...
        if (layer === 'W' && category === 'BCH' && subcategory === 'SUN') subcategoryNum = '001';
        if (layer === 'W' && category === 'BCH' && subcategory === 'TRO') subcategoryNum = '002';
        if (layer === 'W' && category === 'HIP' && subcategory === 'BAS') subcategoryNum = '001';
        if (layer === 'S' && category === 'POP' && subcategory === 'HPM') subcategoryNum = '007';
  ...
  describe('convertMFAtoHFN', () => {
    it('should correctly convert MFA to HFN', () => {
      // Mock implementation
      (taxonomyServiceEnhanced.convertMFAtoHFN as jest.Mock).mockImplementation((mfa) => {
        if (mfa === '5.001.001.001') return 'W.BCH.SUN.001';
        if (mfa === '2.001.007.001') return 'S.POP.HPM.001';
        return null;
      });
      
      expect(taxonomyService.convertMFAtoHFN('5.001.001.001')).toBe('W.BCH.SUN.001');
      expect(taxonomyService.convertMFAtoHFN('2.001.007.001')).toBe('S.POP.HPM.001');
    });
